...
import { BridgeTransaction, BridgeStatus } from '@/hooks/useCrossChainSwapExecution';
...
const getExplorerUrl = (chainIndex: string, txHash: string): string => {
  const explorers: Record<string, string> = {
    ...
    '195': 'https://tronscan.org/#/transaction/',
    '784': 'https://suiscan.xyz/tx/',
    '607': 'https://tonviewer.com/transaction/',
...
export function BridgeProgress({ transaction, onClose }: BridgeProgressProps) {
  const { status, fromChain, toChain, fromToken, toToken, fromAmount, toAmount, sourceTxHash, bridgeName, estimatedTime, startTime, error } = transaction;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
          <div className="flex items-center gap-2">
            <img src={fromToken.tokenLogoUrl} alt={fromToken.tokenSymbol} className="w-6 h-6 rounded-full" />
            <div>
              <div className="font-mono text-sm">{parseFloat(fromAmount).toFixed(4)}</div>
              <div className="text-xs text-muted-foreground">{fromToken.tokenSymbol} on {fromChain.name}</div>
            </div>
          </div>
          <div className="text-muted-foreground">→</div>
          <div className="flex items-center gap-2">
            <img src={toToken.tokenLogoUrl} alt={toToken.tokenSymbol} className="w-6 h-6 rounded-full" />
            <div className="text-right">
              <div className="font-mono text-sm">{toAmount ? parseFloat(toAmount).toFixed(4) : '...'}</div>
              <div className="text-xs text-muted-foreground">{toToken.tokenSymbol} on {toChain.name}</div>
            </div>
          </div>
        </div>
